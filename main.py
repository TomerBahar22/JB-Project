...
def home():
    ...
    )  # added the current user vpc to a previous empty argument
    vpc_data = [
        [
            {
                "VPC ID": vpc["VpcId"],
    ...
    )  # added the current user lb to a previous empty argument
    lb_data = [
    ...
    amis = ec2_client.describe_images(
        Owners=["self"]
    )  # added the current user images to a previous empty argument
    ami_data = [
